        ...
        std::vector<unsigned> intt(std::vector<unsigned> &v)
        {
            ...
            for (unsigned k = 0; k < length; k++)
            {
                ...
                for (unsigned j = 0; j < length; j++)
                {
                    ...
                    res = ((res % m) + (v_temp[j] % m) * modInverseIT(iterativePowMod(ur, j * k, m), m)) % m;
                    std::cout << res << ", ";
                }
                std::cout << "k is :" << k << std::endl;
                v[k] = res;
                                     ...
                                     md_space_t verbose = 0)
        {
            ...
            for (md_space_t k = 0; k < length; k++)
            {
                ...
                for (md_space_t j = 0; j < length; j++)
                {
                    ...
                    res = ((res % m) + ((v_temp[j] % m) * modInverseWithPowModIT(iterativePowMod(ur, j * k, m), m) % m)) % m;
...
#define intt65536 dft::modspace2k::intt<((dft::modspace2k::md_space_t)1 << 16) + 1>
        ...
        namespace iterative
        {
            ...
            {
                return !((UINT_MAX << order) & i) * // sharp bitinverse
                       (((~i) << ((sizeof(unsigned) << 3) - order)) >> ((sizeof(unsigned) * 8) - order));
            }
            namespace vec65537
            {
                std::vector<md_space_t> fft(std::vector<md_space_t> c)
                {
                    if (IsPowerOfTwo(c.size()))
                    {
                        unsigned o = std::log2(c.size());
                        // conversion to complex<double> array
                        std::vector<md_space_t> r;
                        for (unsigned i = 0; i < c.size(); i++)
                        {
                            r.push_back(md_space_t(c[bitreverse(i, o)])); // fed with bitinverse index
                            // r.push_back(md_space_t(c[i])); //fed with bitinverse index
                        }
                        ...
                        for (unsigned i = 0; i < o; i++)
                        {
                            ...
                            {
                                md_space_t multiplier(1, 0);
                                // extract uneven and even
                                for (unsigned k = 0; k < Partitionsize / 2; k++) // 2
                                {

                                    md_space_t t = multiplier * r[Partition + k + Partitionsize / 2];
                                    md_space_t u = r[Partition + k];
                                    r[k + Partition] = u + t;
                ...
                std::vector<md_space_t> ifft(std::vector<md_space_t> c)
                {
                    if (IsPowerOfTwo(c.size()))
                    {
                        unsigned o = std::log2(c.size());
                        ...
                        for (unsigned i = 0; i < o; i++)
                        {
                            ...
                            for (unsigned Partition = 0; Partition < N_SIZE; Partition += Partitionsize)
                            {
                                md_space_t multiplier(1, 0); // twiddle
                                // extract uneven and even
                                for (unsigned k = 0; k < Partitionsize / 2; ++k) // 2
                                {
                            ...
                            ur <<= 1; //duplicate the root of unity.
                            Partitionsize <<= 1;
                        }
                ...
                void benchmark(const unsigned order)
                {
                    ...
                    std::cout << "\tcomparing some elements: " << std::endl
                              << "\t\toriginal sample: ";
            ...
            namespace farr
            {
                ...
                std::complex<double> *fft(std::complex<double> const *c, const unsigned N_SIZE)
                {
                ...
                std::complex<double> *ifft(std::complex<double> const *c, const unsigned N_SIZE)
                {
                    if (IsPowerOfTwo(N_SIZE))
                    {
